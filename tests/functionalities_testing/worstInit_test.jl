...
# u_init, domain = extendInitialDataToK(Nx, sol)
# #plot(u_init[:,1], label="water height")
# plot(u_init[:,1] .+ domain.sourceVec, label="surface")
# plot!(domain.sourceVec, label="z")
...
# fv_sol = fv_solve(domain, u_init, eq, scheme)
# display(plot_fv_sol(fv_sol, eq, nb_plots=2))

# solEnt = optimize_for_entropy(u_init, domain, eq, scheme)#, modifiedDataType=maxK())
