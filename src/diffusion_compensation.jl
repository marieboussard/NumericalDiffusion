function compute_G_mid_f_delta(f_delta::AbstractArray{T}, sol::OptForEntropySol) where T
...
function compute_G_mid(A::AbstractArray{T}, sol::OptForEntropySol) where T
    compute_G_mid_f_delta(f_delta_from_A(A, sol), sol)
end
...
function diffusion_from_A(A::AbstractArray{T}, sol::OptForEntropySol) where T
    ...
    [get_eta(equation, up[i,:]; z=z[i])[1] - get_eta(equation, u[i,:]; z=z[i])[1] for i in 1:length(u[:,1])] + dt / dx * (G_mid[2:end] - G_mid[1:end-1])
end
...
function find_optimal_A(sol::OptForEntropySol)
    @show A_init, _p = zero(sol.Gopt), 0.0
    ...
    prob = OptimizationProblem(optprob, A_init, _p, lcons = [-10.0], ucons = [0.0]) # -Inf
