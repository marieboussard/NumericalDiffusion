...
function constraint_AG(AG::AbstractVector{T}, sol::OptForEntropySol) where T
    L = size(AG)[1] + 1
    A, G = AG[1:Int(L/2),:], vcat(AG[Int(L/2)+1:end,:], AG[Int(L/2)+1,:])
...
function find_optimal_AG(sol::OptForEntropySol)
    #Nx, - = size(sol.u_approx[1])
    #@show AG_init, _p = zero(solEnt.Gopt), 0.0
    AG_init = vcat(zero(sol.Gopt), sol.Gopt[begin:end-1])
    ...
    L = size(AG_init)[1] + 1
    OptAGSol(solAG.u[1:Int(L/2),:], vcat(solAG.u[Int(L/2)+1:end,:], solAG.u[Int(L/2)+1,:]))
end