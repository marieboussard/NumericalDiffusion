...
function lsEntropicConsistentG(domain::Domain, equation::Equation, method::FVMethod, u0, Gc::NumEntFlux; modifiedDataType::ModifiedDataType=meanK_multidim(get_sL(method), get_sR(method)))
    #dt = computeTimeStep(method, equation, domain, u0)
    dt = domain.Tf
    @show l, L = compute_multidim_bounds(u0, domain.Nx, domain.dx, dt, equation, domain, method, modifiedDataType)
    @show gc = giveNumFlux(equation.source, Gc, equation, u0)[end-1]
    ...
    Gopt[begin] = Gopt[end]
    LsGsol(domain, equation, method, u0, Gc, modifiedDataType, l, L, is_solved_and_feasible, Gopt, objective_value(model))
end