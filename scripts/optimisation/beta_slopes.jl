...
function compute_beta_slope(Nxvec::Vector{Int}, equation::Equation; xmin::Real=-2, xmax::Real=2, t0::Real=0.0, tf::Real=0.4, CFL_factor::Real=0.5, weights=AbsWeights(), time_scheme::TimeScheme=Euler(), space_scheme::SpaceScheme=Rusanov(), ent_numflux::AbstractEntNumFlux=CenteredG(), maxiter_uzawa::Int=1000, maxiter_newton::Int=1000, bound_mode=SingleBound(), printing=false)
    ...
    for k in 1:N
        ...
        for n in 1:niter
            ...
            # UZAWA
            optsol = optimize_uzawa(Gc, A, b; W=W, maxiter=maxiter_uzawa, eps=1e-12, printing=printing)
            ...
            pend, wend, niter = newton_lcp(M, -q, optsol.popt, w0; maxiter=maxiter_newton, printing=printing)
