...
schemeRoe = FVScheme(Euler(), Roe(CFL_factor))
...
# Rusanov
solAsym = optimize_for_entropy(u0, omega, burgers(), scheme, modifiedDataType=AsymmetricModifiedData(), method=LBFGS(), autodiff=:forward)
solAsym.label = "Asymmetric"
push!(solEntRusanovVec, solAsym)

# Roe
solAsymRoe = optimize_for_entropy(u0, omega, burgers(), schemeRoe, modifiedDataType=AsymmetricModifiedData(), method=LBFGS(), autodiff=:forward)
solAsymRoe.label = "Asymmetric"
push!(solEntRoeVec, solAsymRoe)

for i in eachindex(KVec)
    # Rusanov
    solEnt = optimize_for_entropy(u0, omega, burgers(), scheme, modifiedDataType=KVec[i], method=LBFGS(), autodiff=:forward)
    solEnt.label = labelKVec[i]
    push!(solEntRusanovVec, solEnt)
    push!(asymGapVec, abs.((solEnt.Dopt .- solAsym.Dopt) ./ solAsym.Dopt))

    # Roe
    solEnt = optimize_for_entropy(u0, omega, burgers(), schemeRoe, modifiedDataType=KVec[i], method=LBFGS(), autodiff=:forward)
    solEnt.label = labelKVec[i]
...
plt2 = plot(size=(900, 600), margin=0.5Plots.cm, legend=:topleft,
    ...
    guidefontsize=21,
    tickfontsize=18)
sol = fv_solve(omega, u0, burgers(), schemeRoe)
x = sol.domain.x
u_exact = [uexact_burgers_article(xi, sol.domain.Tf) for xi in x]
plot!(x, sol.u_approx[end], label="Approx", lw=2)
plot!(x, u_exact, label="Exact", lw=2)
