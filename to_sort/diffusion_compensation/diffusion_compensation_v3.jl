...
function modified_scheme_step_mu(mu::AbstractVector{T}, sol::OptForEntropySol) where T
    sol.u_approx[end] .+ mu
end

function compute_modified_bounds_mu(mu::AbstractVector{T}, sol::OptForEntropySol) where T
...
function find_optimal_muG(sol::OptForEntropySol)
    ...
    @show 2-1
    @show typeof(sol.Gopt)
    @show zero(sol.Gopt[1:end-1])
    @show vcat(zero(sol.Gopt[1:end-1]), sol.Gopt[1:end-1])
    @show muG_init, _p = vcat(zero(sol.Gopt[begin:end-1]), sol.Gopt[begin:end-1]), 0.0
    ...
    prob = OptimizationProblem(optprob, muG_init, _p, lcons = [-1000.0], ucons = [0.0])
